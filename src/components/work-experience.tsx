...
export type ExperiencePositionItemType = {
  ...
  /** Where the role was based (e.g., "Bengaluru, India · Remote") */
  location?: string
...
export type WorkExperienceProps = {
  ...
  /** Show only this many items initially, with a "View more" toggle for the rest. */
  initialCount?: number
}
...
export function WorkExperience({
  ...
  const canCollapse =
    typeof initialCount === "number" && experiences.length > initialCount
  const visible =
    canCollapse && !expanded ? experiences.slice(0, initialCount) : experiences
    ...
    <div className={cn("bg-background px-4 text-foreground", className)}>
      ...
      {canCollapse && (
        <button
          ...
          className="not-prose mt-2 text-xs text-muted-foreground underline-offset-4 hover:text-foreground hover:underline"
        >
          {expanded
            ? "View less"
            : `View ${experiences.length - initialCount!} more`}
        </button>
      )}
    </div>
  )
    ...
    <div className="space-y-4 py-4">
      <div className="not-prose flex items-center gap-3">
        ...
        {experience.isCurrentEmployer && (
          ...
          >
            <span className="absolute inline-flex size-2.5 animate-ping rounded-full bg-emerald-500 opacity-40" />
